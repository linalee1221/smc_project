	...
	var Addflowevent = {
		'click #seluser' : function(e, val ,row ){
			...
			$.ajax({
				...
				$('#DepartNM').text(response.rows[0].DESCRIPTION); // 부서
				$('#UserID').text(response.rows[0].CN); // 사번
				$('#UserNM').text(row.user_nm); // 성명
				$('#EnterDate').text(response.rows[0].IBSA_YMD); // 입사일
 				$('#Jikjong').text(response.rows[0].JIKJONG_CODE_NAME); // 직종
 		 		$('#Jikgub').text(response.rows[0].JIKGEUB_CODE_NAME); // 직급
...
<script>
	...
	$('#FM00000300_confirm').unbind().on('click', function() {
		/*-- TODO: validation check후 완료 시 HGW_APPR_INFO에 데이터 삽입 후 다이얼로그 창 닫기 --*/
	
		var formData = $('#FORM_FM00000300').serializeObject();
		var formDatas = [];
		...
		// 직급 : jikgub, 성명 : 기안자
		_$hwpObj.insertSignData('jikgub', HGW_APPR_BANK.loginVO.memb_rank);
	...
	$('#FM00000300_reset').unbind().on('click', function() {
...
<!-- 버튼영역 -->
	<div class="btn-area-wrap" style="margin-top: 20px;">
		<button type="button" id="FM00000300_confirm" class="btn_large_basic_setting btn_large_accent_box"><spring:message code='button.confirm'/></button>
		<button type="button" id="FM00000300_reset" class="btn_large_basic_setting btn_large_normal_box"><spring:message code='button.reset'/></button>
	</div>
<!--// 버튼영역 -->
</form>
